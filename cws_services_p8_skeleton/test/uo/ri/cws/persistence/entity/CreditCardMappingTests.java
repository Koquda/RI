...
public class CreditCardMappingTests {
    ...
    public void testAllFieldsPersisted() {
        CreditCard restored =
                unitOfWork.findById(CreditCard.class, creditCard.getId());

        assertEquals(creditCard.getId(), restored.getId());
        assertEquals(creditCard.getAccumulated(), restored.getAccumulated(),
                0.001);
    ...
    public void testRepeated() {
        CreditCard repeated =
                new CreditCard(creditCard.getNumber(), "another" + "-card-type",
                        LocalDate.now());
