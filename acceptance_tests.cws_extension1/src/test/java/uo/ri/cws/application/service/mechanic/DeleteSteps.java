...
public class DeleteSteps {
    ...
    public void a_mechanic() throws BusinessException {
        mechanic =
                new MechanicUtil().withDni(randomDni()).withName(randomName())
                        .withSurname(randomSurname()).register().get();
    ...
    public void a_mechanic_with_work_orders_registered()
    ...
    public void i_try_to_remove_the_mechanic() {
        if (mechanic == null) mechanic = (MechanicDto) ctx.get(Key.MECHANIC);
        tryDeleteAndKeepException(mechanic.dni);

    }

    @When("I try to remove a mechanic with null argument")
    public void i_try_to_remove_a_mechanic_with_null_argument()
            throws BusinessException {
