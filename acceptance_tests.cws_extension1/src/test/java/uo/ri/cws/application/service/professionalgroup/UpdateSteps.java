...
public class UpdateSteps {
    ...
    public void i_try_to_add_a_professional_group_with_null_id() {
        ProfessionalGroupBLDto dto =
                new ProfessionalGroupUtil().withName(null).get();
    ...
    public void i_try_to_add_a_professional_group_with(String arg) {
        ProfessionalGroupBLDto dto =
                new ProfessionalGroupUtil().withName(arg).get();
        tryUpdateAndKeepException(dto);
    ...
    public void i_try_to_update_a_professional_group_with_negative_triennium() {
        ProfessionalGroupBLDto dto =
                new ProfessionalGroupUtil().withTriennium(-10.0).get();
        tryUpdateAndKeepException(dto);

    }

    @When("I try to update a professional group with negative productivity " +
            "plus")
    public void i_try_to_update_a_professional_group_with_negative_productivity_plus()
            throws BusinessException {
        ProfessionalGroupBLDto dto =
                new ProfessionalGroupUtil().withProductivity(-10.0).get();
        tryUpdateAndKeepException(dto);
    ...
    public void i_update_a_professional_group() throws BusinessException {
        ...
        FindProfessionalGroupByIdSqlUnitOfWork unit2 =
                new FindProfessionalGroupByIdSqlUnitOfWork(updated_dto.id);
        unit2.execute();
        found_dto = unit2.get();
    ...
    public void version_increases(Integer int1) {
        assertEquals(found_dto.version, before.version + 1);

    }

    @Then("Name does not change")
    public void name_does_not_change() {
        assertEquals(0, found_dto.name.compareTo("I"));

    }

    @Then("Triennium and productivity are updated")
    public void triennium_and_productivity_are_updated() {
        assertEquals(found_dto.productivityRate, updated_dto.productivityRate,
                0.0);
        assertEquals(found_dto.trieniumSalary, updated_dto.trieniumSalary, 0.0);
