...
public class InvoicingSteps {
    ...
    private final List<WorkOrderDto> allWorkorders = new ArrayList<>();
    private final List<WorkOrderDto> onlyFinishedWorkorders = new ArrayList<>();
    private final List<InvoicingWorkOrderDto> invoicingWorkOrders =
    ...
    private void createWorkOrderWithState(String state) {
        ...
        workorder = new WorkOrderUtil().forVehicle(vehicle.id).withState(state)
                .withAmount(randomAmount(leftLimit, rightLimit)).register()
                .get();
    ...
    @When("I try to create an invoice for a list and one of the strings is " +
            "empty")
...
//    }
    ...
    public void iCreateAnInvoiceForTheWorkorders() throws BusinessException {
        if (workordersIds.isEmpty()) {
            workordersIds = ((List<WorkOrderDto>) this.ctx.get(
                    TestContext.Key.WORKORDERS)).stream()
...
//    public void iSearchNotInvoicedWorkordersByDni ( ) throws
//    BusinessException {
//
...
//    }
    ...
    public void iGetOnlyFinishedWorkorders() {
        assertEquals(this.invoicingWorkOrders.size(),
                this.onlyFinishedWorkorders.size());
        /* ids match */
        Map<String, WorkOrderDto> map = this.onlyFinishedWorkorders.stream()
                .collect(Collectors.toMap(dto -> dto.id, dto -> dto));
        List<InvoicingWorkOrderDto> hasDiffId = invoicingWorkOrders.stream()
    ...
    public void anInvoiceIsCreated() {
        ...
        /* with proper number */
        /* with proper amount */
        assertEquals(found.total, expectedInvoice.total, 0.0);

        /* with proper state */
        assertEquals(found.state, expectedInvoice.state);

        /* workorders are updated */
        List<WorkOrderDto> updated = new ArrayList<>();
        for (String id : workordersIds) {
            FindWorkOrderSqlUnitOfWork find =
                    new FindWorkOrderSqlUnitOfWork(id);
            find.execute();
            updated.add(find.get());
        }

        /* to proper state */
        assertTrue(updated.stream().allMatch(
                wo -> wo.state.equals(WorkOrderState.INVOICED.toString())));
