...
public class UpdateSteps {
    ...
    public void i_try_to_update_a_non_existent_contract_type() {
        ContractTypeDto dto =
                new ContractTypeUtil().withName(UUID.randomUUID().toString())
                        .get();
        tryUpdateAndKeepException(dto);
    }
    ...
    public void i_update_a_contract_type() throws BusinessException {
        ...
        FindContractTypeByIdSqlUnitOfWork unit2 =
                new FindContractTypeByIdSqlUnitOfWork(before.id);
        unit2.execute();
        after = unit2.get();
    ...
    public void i_update_a_contract_type_repeatedly() throws BusinessException {
        updated_dto = new ContractTypeDto();
        updated_dto.name = "PERMANENT";
        updated_dto.compensationDays = 10;
    ...
    public void version_increases(Integer arg) {
        ...
        assertEquals(before.version + arg, (long) after.get().version);

    }

    @Then("Compensation days field is updated")
    public void triennium_and_productivity_are_updated() {
        assertTrue(after.get().compensationDays - updated_dto.compensationDays <
                Double.MIN_NORMAL);
    }
    ...
    public void name_does_not_change() {
        assertEquals(0, after.get().name.compareTo("PERMANENT"));
