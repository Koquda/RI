...
public class MechanicSteps {
    ...
    private void processRow(Map<String, String> row) {
        MechanicDto mDto = registerMechanic(row.get("dni"), row.get("name"),
                row.get("surname"));
        ctx.put(Key.MECHANIC, mDto);

        ContractDto cDto = registerContractForMechanic(mDto, row.get("type"),
                row.get("group"), row.get("startDate"), row.get("endDate"),
                row.get("state"), row.get("pay"));
    ...
    private void processRowTerminated(Map<String, String> row) {
        ...
        ContractDto c = registerContractForMechanic(mDto, row.get("type"),
                row.get("group"), row.get("startDate"), row.get("endDate"),
                "TERMINATED", row.get("pay"));
        ctx.put(Key.TERMINATED, c);
    }

    private MechanicDto registerMechanic(String dni, String name,
                                         String surname) {
        return new MechanicUtil().withDni(dni).withName(name)
                .withSurname(surname).register().get();
    }
    ...
    private void insertContractsForMechanic(MechanicDto m, DataTable dataTable)
            throws BusinessException {
        List<Map<String, String>> table = dataTable.asMaps();
        ...
        for (Map<String, String> row : table) {
            ...
            dto = registerContractForMechanic(m, type, group, startDate,
                    endDate, state, pay);
            cs.add(dto);
        }
    ...
    public void mechanicWithContractInForce(DataTable dataTable)
            throws BusinessException {
        Map<String, String> row = dataTable.asMaps().get(0);
        MechanicDto mDto = registerMechanic(row.get("dni"), row.get("name"),
                row.get("surname"));
        ...
        ContractDto c = registerContractForMechanic(mDto, row.get("type"),
                row.get("group"), LocalDateConverter.asString(sixMonthsAgo),
                row.get("endDate"), // no end date if null
                "IN_FORCE", row.get("pay"));
        ctx.put(Key.INFORCE, c);
    }

    @Given("the following mechanic with a contract in force shorter than a " +
            "year")
    public void mechanicWithContractInForceShorterThanOneYear(
    ...
    public void mechanicWithAContractInForceForOneYear(DataTable dataTable)
            throws BusinessException {
        Map<String, String> row = dataTable.asMaps().get(0);
        MechanicDto mDto = registerMechanic(row.get("dni"), row.get("name"),
                row.get("surname"));
        ...
        LocalDate oneYearMonthAgo = LocalDate.now().minusMonths(1).minusYears(1)
                .with(TemporalAdjusters.firstDayOfMonth());

        ContractDto c = registerContractForMechanic(mDto, row.get("type"),
                row.get("group"), LocalDateConverter.asString(oneYearMonthAgo),
                null, // no end date
                "IN_FORCE", row.get("pay"));
        ctx.put(Key.INFORCE, c);
    }
    ...
    private ContractDto registerContractForMechanic(MechanicDto m, String type,
                                                    String group,
                                                    String startDate,
                                                    String endDate,
                                                    String state,
        ...
        ContractTypeDto ct =
                new ContractTypeUtil().findContractType(type).get();
        ProfessionalGroupBLDto pg =
                new ProfessionalGroupUtil().findProfessionalGroup(group).get();
        double pay = (payment == null || payment.isBlank()) ? 1000.0 :
                Double.parseDouble(payment);

        LocalDate start = (startDate == null) ? LocalDate.now().plusMonths(1)
                .with(TemporalAdjusters.firstDayOfMonth()) :
                LocalDateConverter.convert(startDate);

        LocalDate end = (ct.name.equals("FIXED_TERM")) ?
                LocalDateConverter.convert(endDate) : null;

        return new ContractUtil().unique().forMechanic(m).withType(ct)
                .withGroup(pg).withState(state).withStartDate(start)
                .withEndDate(end).withAnnualWage(pay).register().get();
    }

//	private ContractDto addTerminatedContractForMechanic(MechanicDto m, String
//	type, String group, String startDate,
//			String endDate, String payment) throws BusinessException {
//		ContractTypeDto ct = new ContractTypeUtil().findContractType(type)
//		.get();
...
//				? LocalDate.now().plusMonths(1).with(TemporalAdjusters
//				.firstDayOfMonth())
//				: LocalDateConverter.convert(startDate);
//		
//		LocalDate end = (ct.name.equals("FIXED_TERM")) 
...
//	}
    ...
    public void iTryToAddANewMechanicWithNullDni() {
        ...
        MechanicDto dto =
                new MechanicUtil().withDni(null).withName(randomName())
                        .withSurname(randomSurname()).get();
    ...
    public void iTryToAddANewMechanicWithSameDni() throws BusinessException {
        ...
        MechanicDto dto = new MechanicUtil().withDni(
                ((MechanicDto) (ctx.get(Key.MECHANIC))).dni).get();
    ...
    public void iTryToUpdateNonExistingMechanic() throws BusinessException {
        MechanicDto dto =
                new MechanicUtil().withId("mechanic-does-not-exist").get();
    ...
    public void iTryToUpdateTheMechanicToDniNameAndSurname(String name,
                                                           ...
                                                           String dni)
            throws BusinessException {
        MechanicDto dto = new MechanicUtil().loadById(
                ((MechanicDto) (ctx.get(Key.MECHANIC))).id).get();
...
//	public void iTryToFindAMechanicByANonExistentDni() throws
//	BusinessException {
...
//    }
    ...
    public void iLookForMechanic() throws BusinessException {
        optional = service.findMechanicById(
                ((MechanicDto) (ctx.get(Key.MECHANIC))).id);
    }
...
//    }
    ...
    public void theMechanicResultsUpdatedForNameAndSurnameOnly()
            throws BusinessException {
        MechanicDto loaded = new MechanicUtil().loadById(
                ((MechanicDto) (ctx.get(Key.MECHANIC))).id).get();
    ...
    public void iGetMechanic(DataTable table) {
        ...
        String mString = dto.dni + "," + dto.name + "," + dto.surname;
