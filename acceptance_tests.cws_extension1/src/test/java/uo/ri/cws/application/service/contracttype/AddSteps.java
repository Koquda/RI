...
public class AddSteps {
    ...
    public void i_register_a_new_professional_group(DataTable table)
        ...
        ContractTypeDto ct = new ContractTypeUtil().withName(row.get("name"))
                .withDays(Double.parseDouble(row.get("days"))).get();
    ...
    public void the_professional_group_is_added(DataTable table) {
        ...
        FindContractTypeByIdSqlUnitOfWork finder =
                new FindContractTypeByIdSqlUnitOfWork(this.ct_dto.id);
        finder.execute();
        Optional<ContractTypeDto> found = finder.get();
        assertTrue(found.isPresent());
        assertEquals(1L, (long) found.get().version);
        String string2 = found.get().name + "," + found.get().compensationDays;
