...
public class AddSteps {
    ...
    private final ContractUtil cUtil = new ContractUtil();
    private final PayrollUtil pUtil = new PayrollUtil();
    ...
    public void i_register_this_contract_for_this_mechanic(DataTable dataTable)
        ...
        ContractDto previous =
                new ContractUtil().findContractInForceForMechanic(m.id).get();

        if (previous != null) { // there were a previous contract in force
            FindContractByIdSqlUnitOfWork x =
                    new FindContractByIdSqlUnitOfWork(previous.id);
            x.execute();
        ...
        double pay =
                (p == null || p.isBlank()) ? 1000.0 : Double.parseDouble(p);
        pay = Round.twoCents(pay);

        ContractDto contractDto =
                new ContractUtil().unique().forMechanic(m).withType(ct)
                        .withGroup(pg).withAnnualWage(pay).get();
    ...
    public void contract_version_is(Integer n) {
        assertEquals(inForce.version, (int) n);

    }

    @Then("previous contract is terminated")
    public void previous_contract_is_terminated() throws BusinessException {
        terminated = (ContractDto) ctx.get(Key.TERMINATED);
    ...
    @Then("there is a contract in force for the mechanic with the following " +
            "data")
    public void there_is_a_contract_in_force_for_the_mechanic(
        ...
        assertEquals(inForce.dni, row.get("dni"));
        assertEquals(inForce.professionalGroupName, row.get("group"));
        assertEquals(inForce.contractTypeName, row.get("type"));
        assertEquals(inForce.state, ContractState.IN_FORCE);
        assertEquals(inForce.startDate, initDate);
        if (inForce.contractTypeName.equals("FIXED_TERM")) {
            assertEquals(inForce.endDate, endDate);
        }
    ...
    public void there_is_no_settlement() {
        assertEquals(0.0, inForce.settlement, 0.0);
    }
    ...
    public void start_date_is_first_day_next_month() {
        ...
        assertEquals(inForce.startDate, firstDayofNextMonth);
    }
    ...
    public void settlement_is_calculated() {
        ...
        assertEquals(terminated.settlement, settlement, 0.0);
    ...
    public void i_try_to_add_a_contract_for_a_non_existing_mechanic() {
        ...
        ContractDto c =
                cUtil.unique().forMechanic(m).withType(ct).withGroup(pg).get();
    ...
    public void i_try_to_add_a_contract_with_start_date_not_later_than_today() {
        MechanicDto m = (MechanicDto) ctx.get(Key.MECHANIC);
        ContractTypeDto ct =
                new ContractTypeUtil().findContractType("FIXED_TERM").get();
        ProfessionalGroupBLDto pg =
                new ProfessionalGroupUtil().findProfessionalGroup("I").get();
        ContractDto c =
                cUtil.unique().forMechanic(m).withGroup(pg).withType(ct).get();
    ...
    @When("I try to add a contract with wrong fields {string} {string} " +
            "{double}")
    ...
    @When("I try to add a contract with null end date for FIXED_TERM contract" +
            " type")
    ...
    private double calculateDailyGrossWageLastYear(ContractDto existing) {
        FindPayrollsByContractLastYearSqlUnitOfWork unit =
                new FindPayrollsByContractLastYearSqlUnitOfWork(existing.id);
        unit.execute();
        List<PayrollBLDto> lastYear = unit.get();

        double monthlyEarnings = lastYear.stream()
                .map(pr -> pr.bonus + pr.monthlyWage + pr.productivityBonus +
                        pr.trienniumPayment).reduce(0.0, Double::sum);
//	monthlyEarnings = trunc2Dec(monthlyEarnings);
    ...
    private double findCompensationDays(ContractDto arg) {
        FindContractTypeByIdSqlUnitOfWork unit =
                new FindContractTypeByIdSqlUnitOfWork(arg.contractTypeName);
        unit.execute();
