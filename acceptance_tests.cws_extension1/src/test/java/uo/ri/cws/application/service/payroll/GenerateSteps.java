...
import uo.ri.cws.application.service.util.*;
...
public class GenerateSteps {
    private final PayrollService service = Factory.service.forPayrollService();
    ...
    public void zero_payrolls_are_generated() {
        ...
        assertEquals(0, this.generatedPayslips);
    ...
    @Then("one payroll is generated with {double}, {double}, {double}, " +
            "{double}, {double}, {double}")
    public void one_payroll_is_generated_with(Double moneyMonth, Double bonus,
                                              Double productivity, Double tri,
                                              Double tax, Double nic) {
        List<PayrollBLDto> founds = findGeneratedPayrolls();
        this.generatedPayslips = founds.size();
        assertEquals(1, this.generatedPayslips);
        PayrollBLDto found = founds.get(0);
        assertEquals(0, Double.compare(found.monthlyWage, moneyMonth));
        assertEquals(0, Double.compare(found.bonus, bonus));
        assertEquals(0, Double.compare(found.productivityBonus, productivity));
        assertEquals(0, Double.compare(found.trienniumPayment, tri));
        assertEquals(0, Double.compare(found.incomeTax, tax));
        assertEquals(0, Double.compare(found.nic, nic));
        assertEquals(1L, found.version);
    }

    @Then("{int} payrolls are generated with {double}, {double}, {double}, " +
            "{double}, {double}, {double}")
    public void payrolls_are_generated_with(Integer howMany, Double moneyMonth,
                                            Double bonus, Double productivity,
                                            Double tri, Double tax,
                                            Double nic) {
        List<PayrollBLDto> founds = findGeneratedPayrolls();
        this.generatedPayslips = founds.size();
        assertEquals(this.generatedPayslips, (int) howMany);
        if (howMany > 0) {
            PayrollBLDto found = founds.get(0);
            assertEquals(0, Double.compare(found.monthlyWage, moneyMonth));
            assertEquals(0, Double.compare(found.bonus, bonus));
            assertEquals(0,
                    Double.compare(found.productivityBonus, productivity));
            assertEquals(0, Double.compare(found.trienniumPayment, tri));
            assertEquals(0, Double.compare(found.incomeTax, tax));
            assertEquals(0, Double.compare(found.nic, nic));
            assertEquals(1L, found.version);
    ...
    private List<PayrollBLDto> findGeneratedPayrolls() {
        ...
        FindPayrollsThisMonthSqlUnitOfWork unit =
                new FindPayrollsThisMonthSqlUnitOfWork(currentDate);
        unit.execute();
    ...
    public void two_payrolls_are_generated() {
        List<PayrollBLDto> found = findGeneratedPayrolls();
        this.generatedPayslips = found.size();
        assertEquals(2, this.generatedPayslips);

    }

    @Given("Exactly {int} invoiced workorders for the mechanic with amount " +
            "{double}")
    public void workorders_for_the_mechanic_with_date_and_amount(
        ...
        VehicleDto vehicle =
                new VehicleUtil().withOwner(client.id).register().get();
        for (int i = 1; i <= howmany; i++) {
            LocalDateTime ldt = openWorkorder.withDayOfMonth(i).atStartOfDay();
            workOrderDto =
                    new WorkOrderUtil().forMechanic(m.id).forVehicle(vehicle.id)
                            .withDate(ldt).withAmount(amount)
    ...
    @Given("A contract in force for a mechanic with {string} start date and " +
            "{double} base salary")
    public void a_contract_in_force_for_the_mechanic_with(String date,
        ...
        ContractTypeDto ct =
                new ContractTypeUtil().findContractType("PERMANENT").get();
        ProfessionalGroupBLDto pg =
                new ProfessionalGroupUtil().findProfessionalGroup("I").get();
        LocalDate localDate = LocalDateConverter.convert(date);

        ContractUtil cUtil =
                new ContractUtil().unique().forMechanic(m).withType(ct)
                        .withGroup(pg).withStartDate(localDate)
    ...
    @Given("A contract terminated for a mechanic, with {string} start date, " +
            "{string} end date, {double} base salary")
    public void a_contract_terminated_for_the_mechanic_with_start_date_end_date_base_salary(
        ...
        ContractUtil cUtil =
                new ContractUtil().unique().forMechanic(m).withType(ct)
                        .withGroup(pg).withState("TERMINATED")
                        .withStartDate(first).withEndDate(last)
                        .withAnnualWage(salary).register();

        ctx.put(Key.TERMINATED, cUtil.get());
    }

    @Given("A contract for the mechanic terminated long ago with {double} " +
            "base salary")
    public void a_contract_terminated_long_ago_with_base_salary(Double salary)
            throws BusinessException {
        MechanicDto m = (MechanicDto) ctx.get(Key.MECHANIC);
        ContractTypeDto ct = (ContractTypeDto) ctx.get(Key.CONTRACTTYPE);
        ProfessionalGroupBLDto pg =
                (ProfessionalGroupBLDto) ctx.get(Key.PROFESSIONALGROUP);
    ...
    public void a_contract_type_permanent() {
        ContractTypeUtil util =
                new ContractTypeUtil().findContractType("PERMANENT");
    ...
    public void a_prof_group_1() {
        ProfessionalGroupUtil util =
                new ProfessionalGroupUtil().findProfessionalGroup("I");
