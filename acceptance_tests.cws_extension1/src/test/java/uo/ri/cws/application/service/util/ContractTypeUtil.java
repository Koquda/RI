...
public class ContractTypeUtil {
    ...
    public static boolean match(ContractTypeDto g1, ContractTypeDto g2) {
        ...
        return (match(g1.id, g2.id)) && (g1.version == g2.version) &&
                (match(g1.name, g2.name)) &&
                Math.abs(g1.compensationDays - g2.compensationDays) <
                        Double.MIN_NORMAL;
    }
    ...
    public static boolean newContractTypeIsFound(ContractTypeDto arg,
                                                 List<ContractTypeDto> allFound) {
        for (ContractTypeDto c : allFound)
            if (match(c, arg)) return true;
        return false;
    }
    ...
    public ContractTypeUtil findContractType(String string) {
        FindContractTypeByNameSqlUnitOfWork unit =
                new FindContractTypeByNameSqlUnitOfWork(string);
        unit.execute();
