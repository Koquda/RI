...
public class PayrollUtil {
    ...
    public static boolean match(List<PayrollBLDto> list1,
                                List<PayrollBLDto> list2) {
        ...
        for (int index = 0; index < list1.size(); index++) {
            ...
            if (!PayrollUtil.match(dto1, dto2)) return false;
        }
        return true;
    }
    ...
    public static boolean match(PayrollBLDto pr1, PayrollBLDto pr2) {
        ...
        // null check
        if (pr1 == null) return (pr2 == null);

        return (match(pr1.id, pr2.id)) &&
                (match(pr1.contractId, pr2.contractId)) &&
                (match(pr1.date, pr2.date)) &&
                Math.abs(pr1.monthlyWage - pr2.monthlyWage) <
                        Double.MIN_NORMAL &&
                Math.abs(pr1.bonus - pr2.bonus) < Double.MIN_NORMAL &&
                Math.abs(pr1.trienniumPayment - pr2.trienniumPayment) <
                        Double.MIN_NORMAL &&
                Math.abs(pr1.productivityBonus - pr2.productivityBonus) <
                        Double.MIN_NORMAL &&
    ...
    public static boolean matchPayrollSummaries(List<PayrollSummaryBLDto> list1,
                                                List<PayrollSummaryBLDto> list2) {
        ...
        for (int index = 0; index < list1.size(); index++) {
            s1 = list1.get(index);
            s2 = list2.get(index);
            if (!s1.id.equals(s2.id) || (s1.version != s2.version) ||
    ...
    public static boolean matchPayrollSummaries(PayrollSummaryBLDto s1,
                                                PayrollSummaryBLDto s2) {
        return s1.id.equals(s2.id) && (s1.version == s2.version) &&
                s1.date.equals(s2.date) &&
                Double.compare(s1.netWage, s2.netWage) < 0.001;
    }
