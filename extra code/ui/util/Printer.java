...
import uo.ri.cws.application.business.invoice.Invoice_BLDto;
import uo.ri.cws.application.business.invoice.InvoicingWorkOrder_BLDto;
import uo.ri.cws.application.business.mechanic.Mechanic_BLDto;

public class Printer {

	public static void printMechanic(Mechanic_BLDto m) {
		Console.printf("\t%-36.36s %-9s %-10.10s %-25.25s %-10.2s\n", m.id, m.dni, m.name, m.surname, m.version);
	}

	public static void printMechanics(List<Mechanic_BLDto> list) {

		Console.printf("\t%-36s %-9s %-10s %-25s %-10s\n", "Mechanic identifier", "DNI", "Name", "Surname", "Version");
		for (Mechanic_BLDto m : list)
			printMechanic(m);
	}
